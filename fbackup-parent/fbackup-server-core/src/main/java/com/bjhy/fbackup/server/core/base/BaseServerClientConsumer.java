...
//	private static final List<Class<? extends StaticStoreType>> staticStoreTypeList = ListenerUtil.getListenerClass(StaticStoreType.class);
	...
	/**
	 * 得到FBackupServer接的实现类
	 */
	private static final List<Class<? extends FBackupServer>> fbackupServerList = ListenerUtil.getListenerClass(FBackupServer.class);
...
//	}
	...
	private void deaWithFileTransferConsumer(ClientFileTransfer clientFileTransfer) throws Exception{
		List<DirectoryInfo> directoryList = server.getXmlServer().getDirectoryList();
		for (DirectoryInfo directoryInfo : directoryList) {
			if(StringUtils.isBlank(directoryInfo.getDirectoryType())) {
				continue;
			}
			
			List<FBackupServer> serverList = findByDirectoryType(directoryInfo.getDirectoryType());
			for (FBackupServer fbackupServer : serverList) {
				fbackupServer.dealWithResource(clientFileTransfer, server,directoryInfo);
			}
		}
...
//		}
	 ...
	 * @return
	 */
	private List<FBackupServer> findByDirectoryType(String directoryType){
		List<FBackupServer> serverList = buildFBackupServerInstances();
		
		//内部实现
		List<FBackupServer> fbackupInternal = new ArrayList<FBackupServer>();
		//非内部实现
		List<FBackupServer> nonFbackupInternal = new ArrayList<FBackupServer>();
		
		for (FBackupServer server : serverList) {
			if(!server.directoryType().equals(directoryType)) {
				continue;
			}
			
			FBackupListener fBackupListener = server.getClass().getAnnotation(FBackupListener.class);
			if(fBackupListener != null){
				boolean internal = fBackupListener.isFbackupInternal();
				if(internal){
					fbackupInternal.add(server);
				}else{
					nonFbackupInternal.add(server);
				}
			}else{
				nonFbackupInternal.add(server);
			}
	 ...
	 * @return
	 */
	private List<FBackupServer> buildFBackupServerInstances(){
		List<FBackupServer> instances = new ArrayList<FBackupServer>();
		for (Class<? extends FBackupServer> clazz : fbackupServerList) {
			FBackupServer instance = ExtensionLoader.getInstance(clazz);
			if(instance == null){
				try {
