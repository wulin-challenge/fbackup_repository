 ...
 *
 */
public class FBackupFileCoreDealwith {
	
	private final XmlFbackup xmlFbackup = ExtensionLoader.getInstance(XmlFbackup.class);
	
	private FBackupFileCoreDealwith(){}
	/**
	 * 检查并存储文件
	 * @param fileType
	 * @param oneFile
	 */
	public void checkAndStoreFile(DirectoryInfo directoryInfo,File oneFile,String relativeFilePath){
		...
		if(StringUtils.isNotBlank(relativeFilePath)){
			...
			try {
				...
				if(hasData == 0){
					FileTransferEntity fileTransferEntity = getFileTransferEntity(directoryInfo, oneFile, relativeFilePath);
...
//			LoggerUtils.info("当前节点有数据,通知zookeeper成功!");
	 ...
	 * @return
	 */
	public FileTransferEntity getFileTransferEntity(DirectoryInfo directoryInfo,File oneFile,String relativeFilePath){
		...
		fileTransfer.setDirectoryType(directoryInfo.getDirectoryType());
		fileTransfer.setContent(directoryInfo.getContent());
		fileTransfer.setCustomField(directoryInfo.getCustomField());
	 ...
	 * @return
	 */
	public static FBackupFileCoreDealwith getInstance(){
		FBackupFileCoreDealwith instance = ExtensionLoader.getInstance(FBackupFileCoreDealwith.class);
		if(instance == null){
			instance = new FBackupFileCoreDealwith();
			ExtensionLoader.setInstance(FBackupFileCoreDealwith.class, instance);
		}
		return instance;
	}
}
